...
const Home = () => {
    ...
    return (
        <Fragment>
            ...
            {loading ? <div style={{display: 'flex', justifyContent: 'center', alignItems: 'center', height: '50vh'}}> <svg xmlns="http://www.w3.org/2000/svg" width="3em" height="3em" viewBox="0 0 24 24"><path fill="currentColor" d="M12 2A10 10 0 1 0 22 12A10 10 0 0 0 12 2Zm0 18a8 8 0 1 1 8-8A8 8 0 0 1 12 20Z" opacity="0.5"/><path fill="currentColor" d="M20 12h2A10 10 0 0 0 12 2V4A8 8 0 0 1 20 12Z"><animateTransform attributeName="transform" dur="1s" from="0 12 12" repeatCount="indefinite" to="360 12 12" type="rotate"/></path></svg></div> : 
                     ...
                     {status?.map((item, index) => {
                        return (
                            ...
                            <Col xl={3} lg={6} md={12} xs={12} className="mt-6" key={index}>
                                <StatRightTopIcon icon={item?.status === 'PENDING' ? <Briefcase /> : item?.status === 'COMPLETED' ? <People /> : <ListTask />} status={item?.status} count={item?.count} />
                            </Col>
                            </>
                        )
                    })}
                 <Col xl={3} lg={6} md={12} xs={12} className="mt-6">
                    <StatRightTopIcon icon={wallet?.icon} status={wallet?.title} count={walletBalance} />
                    </Col>  
                </Row>
                <ActiveProjects />
            </Container>
